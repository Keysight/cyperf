...
class RESTasV3:
    ...
    def __sendPost(self, url, payload, customHeaders=None, files=None, debug=True):
        ...
        headers = self.headers if customHeaders is None else customHeaders
        payload = json.dumps(payload) if customHeaders is None else payload
        response = self.session.post('{}{}'.format(self.host, url), headers=headers, data=payload, files=files, verify=False)
        if debug:
        ...
        if response.status_code not in expectedResponse:
            raise Exception('Unexpected response code. Actual: {} Expected: {}'
                            .format(response.status_code, expectedResponse))
        else:
            return response
    ...
    def refresh_access_token(self):
        access_token = self.get_automation_token(
            username=WAP_USERNAME, password=WAP_PASSWORD)
    ...
    def set_license_server(self, licenseServerIP, retries=3, wait=30):
        ...
        try:
            ...
            if retries > 0:
                time.sleep(wait)
                print("Exception occurred, retry {}/3 after {}s".format(retries, wait))
                self.set_license_server(licenseServerIP, retries=retries - 1)
            else:
                raise Exception("Exception when trying to set license server : {}".format(e))
    ...
    def nats_update_route(self, nats_address, retries=3, wait=120):
        ...
        try:
            ...
            if retries > 0:
                time.sleep(wait)
                print("Exception occurred, retry {}/3 after {}s".format(retries, wait))
                self.nats_update_route(nats_address, retries=retries - 1)
            else:
                raise Exception("Exception when trying to update NATS route : {}".format(e))
    ...
    def get_agents(self):
        apiPath = '/api/v2/agent-controller/agents'
        return self.__sendGet(apiPath, 200).json()
    ...
    def get_capture_files(self, captureLocation, exportTimeout=120):
        ...
        while not exportDone:
            ...
            if dt > exportTimeout:
                raise TimeoutError('Failed to download Capture')
    ...
    def get_config_type(self):
        self.config = self.get_session_config()
        config_type = {"traffic": False, "attack": False, "dut": False}
    ...
    def start_test(self, initializationTimeout=60):
        ...
        if response['state'] == 'IN_PROGRESS':
            raise Exception('Test could not start in {} seconds!'.format(initializationTimeout))
        else:
            self.startTrafficTime = self.__getEpochTime()
            InitializationDuration = self.startTrafficTime - self.startTime
            print("Initialization took {} seconds".format(InitializationDuration))
            print("Traffic started at {}".format(self.startTrafficTime))
    ...
    def wait_test_finished(self, timeout=300):
        ...
        while actual_duration < self.testDuration + timeout:
            ...
            print("Test did not stop after timeout {}s. Test status= {}. Aborting...".format(timeout, response['status']))
            self.stop_test()
    ...
    def set_traffic_profile_server_tls(self, version, pr_id=1):
        apiPath = '/api/v2/sessions/{}/config/config/TrafficProfiles/{}/TrafficSettings/DefaultTransportProfile/ServerTLSProfile'.format(self.sessionID, pr_id)
        self.__sendPatch(apiPath, payload={"version": version})
    ...
    def set_attack_profile_server_tls(self, version, pr_id=1):
        apiPath = '/api/v2/sessions/{}/config/config/AttackProfiles/{}/TrafficSettings/DefaultTransportProfile/ServerTLSProfile'.format(self.sessionID, pr_id)
        self.__sendPatch(apiPath, payload={"version": version})
    ...
    def save_config(self, test_name, timeout=10):
        ...
        while timeout > 0:
            ...
            if response:
                if response['state'] == "SUCCESS":
                    print('Test= {} was successfully saved'.format(test_name))
                    return
            else:
                timeout -= counter
                time.sleep(counter)
        else:
            raise Exception("Could not save copy for test= {}".format(test_name))
    ...
    def collect_diagnostics(self, timeout=600):
        ...
        while timeout > 0:
            ...
            raise Exception("Failed to collect diagnostics. Timeout was reached = {}seconds".format(init_timeout))

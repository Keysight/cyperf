...
def binary_search_for_optimal_throughput( results_folder, test_name, config_path, perform_validation, number_of_streams,number_of_trials_in_binary_search, Initiator_mgmt_ip,responder_mgmt_ip, initial_target_throughput_in_gbps, max_allowable_througput_in_gbps, minimum_throughput_limit_in_gbps, tolerance ):
    ...
    print('*******Binary search initiated ******* '.center(width))
    print(f"** BS = {count} ".ljust(width))
    print(f"The minimum throughput value = {minimum_throughput_limit_in_gbps} gbps".ljust(width))
    print(f"The maximum throughput value = {max_allowable_througput_in_gbps} gbps".ljust(width))
    print(f"The target  throughput value = {initial_target_throughput_in_gbps} gbps with tolerance of {tolerance}".ljust (width))
    print('************************************************************************************************************'.ljust(width))
    ...
    while (1):
                    while( target_throughput_acheived ):
                        ...
                        print('*******Binary search initiated ******* '.ljust(width))
                        print(f"** BS = {count} ".ljust(width))
                        print(f"The minimum throughput value = {minimum_throughput_limit_in_gbps} gbps".ljust(width))
                        print(f"The maximum throughput value = {max_allowable_througput_in_gbps} gbps".ljust(width))
                        ...
                        print(f"before entering run() teh list of acheived values = {list_of_achieved_throughput_values}".ljust(100))
                        ...
                        if(not target_throughput_acheived):
                            print(f"******** The target throughput of {present_run_throughput_in_gbps} gbps could not be acheived within the tolerance level".ljust(width))
                    ...
                    while( not target_throughput_acheived ):
                        ...
                        print(f"maximum allowable throughput =  {max_allowable_througput_in_gbps} gbps ".ljust(width))
                        print(f"minimum throughput limit     =  {minimum_throughput_limit_in_gbps} gbps ".ljust(width))
                        ...
                        print(f"The target  throughput value = {initial_target_throughput_in_gbps} gbps".ljust(width))
                        print('************************************************************************************************************'.ljust(width))
                        ...
                        print(f"before entering run()before entering run() teh list of acheived values = {list_of_achieved_throughput_values}".ljust(100))
                        if ( len(list_of_achieved_throughput_values) < 2):
                             run_test()
                        else: 
                            if( resolution < abs (list_of_achieved_throughput_values[-1] - list_of_achieved_throughput_values[-2])):
...
for i in range(number_of_trials):
    ...
    while(list_of_frame_size_in_bytes):
            ...
            if( not target_throughput_acheived):
                print(f"*****  Sorry : Your specified minimum throughput limit with frame size of {frame_size_in_bytes} Bytes could not be acheived ! You need to adjust the minimum throughput flimit or the tolerance value and re-run test  *********".ljust(130))
                print(" continuing with the next frame size ".ljust(130))
                continue
            else:

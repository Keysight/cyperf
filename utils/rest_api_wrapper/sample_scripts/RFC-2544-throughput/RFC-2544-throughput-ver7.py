...
#We have set previous_run__throughput_in_gbps to zero to indicaate that the test never ran prior to executing this script.
...
def binary_search_for_optimal_throughput( results_folder, test_name, config_path,perform_validation,number_of_streams,number_of_trials_in_binary_search,Initiator_mgmt_ip,responder_mgmt_ip,initial_target_throughput_in_gbps,max_allowable_througput_in_gbps,initial_minimum_throughput_limit_in_gbps,tolerance ):
   
    width=120
    #now start the binary algoritm to search the best Throughput 
    #print(f"{name} is {age} years old.".center(width))
    print('*******soumya : I am inside Binary search******* '.center(width))
    print(f"The value of target throughput now is {initial_target_throughput_in_gbps} gbps with tolerance of {tolerance} ".ljust (width))
    print('************************************************************************************************************'.ljust(width))
    ...
    '''if(stop_trial_for_frame_size):
            ...
            print("The present attempted throughput in Gbps is {}".format(initial_target_throughput_in_gbps))
            print("The  attempted throughput in Gbps was acheived ")
            print("********************************************************************")'''
    while (1):
                    while( target_throughput_acheived ):
                        ...
                        print(f"previous throughput = {previous_run_throughput_in_gbps} gbps ".ljust(width))
                        #calculate the target throughput for the next run , as earflier run could not achieve the desired throughput 
                        initial_target_throughput_in_gbps= previous_run_throughput_in_gbps + (max_allowable_througput_in_gbps - previous_run_throughput_in_gbps)/2
                        ...
                        print(f"Present throughput = {initial_target_throughput_in_gbps} gbps".format(initial_target_throughput_in_gbps))
                        print(f"New throughput for this trial = {initial_target_throughput_in_gbps} gbps".ljust(width))
                        ...
                        if(not target_throughput_acheived):
                            print(f"Present throughput of {present_run_throughput_in_gbps} gbps could not be achieved with the tolerance level".ljust(width))
                            print(f"Continue Binary serach ".ljust(width))
                            initial_minimum_throughput_limit_in_gbps = previous_run_throughput_in_gbps
                    ...
                    while( not target_throughput_acheived ):
                        ...
                        #Record the previous run throughput 
                        previous_run_throughput_in_gbps = initial_target_throughput_in_gbps
                        print('*****************Lower half validations *******************'.center(width))
                        print(f"previous throughput =  {previous_run_throughput_in_gbps} gbps ".center(width))
                        #calculate the target throughput for the next run , as earflier run could not achieve the desired throughput 
                        ??initial_target_throughput_in_gbps =  initial_minimum_throughput_limit_in_gbps + (previous_run_throughput_in_gbps - initial_minimum_throughput_limit_in_gbps)/2
                        if( initial_target_throughput_in_gbps < initial_minimum_throughput_limit_in_gbps):
                        ...
                        if(not target_throughput_acheived):
                            print(f"Present throughput of {present_run_throughput_in_gbps} gbps could not be achieved with the tolerance level".ljust(width))
                            print(f"Continue Binary serach ".ljust(width))
                             max_allowable_througput_in_gbps = present_run_throughput_in_gbps
...
#Now values are sanitized we need to start the the first Trail 
...
In case it cannot be acheived we need to stop and inform that he needs to lower the initial_minimum_throughput_limit_in_gbps value
'''
if(initial_minimum_throughput_limit_in_gbps > 0):
    ...
    rest.set_udp_stream_packet_rate(packet_rate)
    run_test()
    #calculate Bytes per second for the targetted throughput for statistics validation @collect_stats_validate_throughput
    ####min_target_throughput_in_Bps= (initial_minimum_throughput_limit_in_gbps*pow(10,9)/8)
    ...
    if( not target_throughput_acheived):
        print("*****  Sorry : Your specified minimum throughput limit coudl not be acheived ! You need to adjust the minimum limit or the tolerance value and re-run test  *********")
    else:
        while (trail_count <= number_of_trials):
    ...
    while(trail_count <= number_of_trials):
        binary_search_for_optimal_throughput("../test_results", "RFC-2544-udp-streaming-throughput","./RFC-2544-throughput",False,number_of_streams,number_of_trials_in_binary_search,Initiator_mgmt_ip,responder_mgmt_ip, initial_target_throughput_in_gbps,max_allowable_througput_in_gbps,initial_minimum_throughput_limit_in_gbps ,tolerance)
        trail_count=trail_count + 1
print("******************** end of Trial #{}".format(trail_count))
...
from cyperf.utils.rest_api_wrapper.lib.REST_WRAPPER import collect_stats_validate_throughput, rest, create_new_config, create_traffic_profile, run_test, collect_stats
import yaml
import time
import pprint
import json
...
def binary_search_for_optimal_throughput( results_folder, test_name, config_path, perform_validation, number_of_streams, Initiator_mgmt_ip,responder_mgmt_ip, initial_target_throughput_in_gbps, max_allowable_througput_in_gbps, minimum_throughput_limit_in_gbps, tolerance , direction_of_stream ):
    ...
    #run_test()
    if ( not  packet_loss_during_test_run( test_duration_in_seconds , packet_loss_tolerance  , direction_of_stream )):
    ...
    while (1):
                    while( target_throughput_acheived ):
                        ...
                        if ( len(list_of_achieved_throughput_values) < 2 ):
                             #run_test()
                             if(packet_loss_during_test_run) :
                                  flag_packet_loss=1
                                  break
                        else: 
...
def extract_sent_packets_at_client(json_data):
    """
    Extracts sent packets and timestamps from the JSON data.

    Args:
    json_data (dict): The JSON data.

    Returns:
    dict: A dictionary with timestamps as keys and sent packets as values.
    """

    # Initialize an empty dictionary
    sent_packets_dict = {}

    # Iterate over each snapshot in the JSON data
    for snapshot in json_data['snapshots']:
        # Get the timestamp and sent packets from the snapshot
        timestamp = snapshot['timestamp']
        sent_packets = int(snapshot['values'][0][12])

        # Add the timestamp and sent packets to the dictionary
        sent_packets_dict[timestamp] = sent_packets

    return sent_packets_dict


def extract_lost_packets_at_client(json_data):
    """
    Extracts lost packets and timestamps from the JSON data.

    Args:
    json_data (dict): The JSON data.

    Returns:
    dict: A dictionary with timestamps as keys and lost packets as values.
    """
    ...
    # Iterate over each snapshot in the JSON data
    for snapshot in json_data['snapshots']:
        # Get the timestamp and lost packets from the snapshot
        timestamp = snapshot['timestamp']
        lost_packets = int(snapshot['values'][0][7])

        # Add the timestamp and lost packets to the dictionary
        lost_packets_dict[timestamp] = lost_packets

    return lost_packets_dict



def extract_sent_packets_at_server(json_data):
    """
    Extracts sent packets and timestamps from the JSON data.
    ...
    for snapshot in json_data['snapshots']:
        # Get the timestamp and sent packets from the snapshot
        timestamp = snapshot['timestamp']
        sent_packets = int(snapshot['values'][0][11])
...
def extract_lost_packets_at_server(json_data):
    """
    Extracts lost packets and timestamps from the JSON data.
    ...
    for snapshot in json_data['snapshots']:
        # Get the timestamp and lost packets from the snapshot
        timestamp = snapshot['timestamp']
        lost_packets = int(snapshot['values'][0][5])
...
def packet_loss_during_test_run( test_duration , packet_loss_tolerance, direction_of_stream ):
    rest.start_test()
    start_time = time.time()
    stat_client= 'client-streaming-statistics'
    stat_server = 'server-streaming-statistics'
    while time.time()-start_time < test_duration:
        if ( direction_of_stream == 'ClientToServer'):
                json_data_sent_packets = rest.get_stats_values(statName=stat_client)
                json_data_lost_packets = rest.get_stats_values(statName=stat_server)
                dict_sent_packets_from_client = extract_sent_packets_at_client(json_data_sent_packets)
                dict_lost_packets_at_server   =   extract_lost_packets_at_server(json_data_lost_packets)
                
                for timestamp in dict_sent_packets_from_client:
                # Extract the value corresponding to the current key
                    sent_packets_at_client = dict_sent_packets_from_client[timestamp]
                    if (timestamp in dict_lost_packets_at_server):
                         lost_packets_at_server = dict_lost_packets_at_server[timestamp]
                         lost = lost_packets_at_server / sent_packets_at_client
                         if (lost > packet_loss_tolerance):
                              print_selective (f"loss greater than tolerance : sent_packets_at_client = {sent_packets_at_client } &  lost_packets_at_server = {lost_packets_at_server} ratio = { lost }")
                              return True
                         else:
                              print_selective (f"loss within tolerance : sent_packets_at_client = {sent_packets_at_client } &  lost_packets_at_server = {lost_packets_at_server} ratio = { lost }")   
                    else: 
                         continue
...
if (max_allowable_througput_in_gbps > line_rate_for_the_media_in_gbps ):
        raise Exception("The maximum allowable throughput cannot be greater than teh Line rate for the Media")

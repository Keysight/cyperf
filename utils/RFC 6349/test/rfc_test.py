...
def connection(logger, gateway_server, gateway_port, ixload_version):
    ix = IxLoad()
    set_logger(logger, level="INFO", message= "Connecting to IxLoad")
    res.append(ix.connect(gateway_server=gateway_server, gateway_port=gateway_port, ixload_version=ixload_version))
    if res[-1]['status'] == 0:
        set_logger(logger, level="INFO", message= "Test Failed: "+ str(res[-1]['log']))
        pytest.fail("Test Failed: "+str(res[-1]['log']))
    return ix
...
def test_script(logger, logger_report):
    ...
    # set_logger(logger, level="INFO", message="Test 4 - Determine the Bottleneck Bandwidth of the network \n -----------------------------------------------------------------\n")
    # get_bottleneck(logger=logger, rxf_file= bottleneck_rxf, supported_file=None, test_duration= 600, chassis_ip=chassis_ip, portlist_per_community=portlist_per_community, client_ip=client_ip, server_ip=server_ip, client_gateway_ip=client_gateway_ip,server_gateway_ip=server_gateway_ip, client_subnet_mask=client_subnet_mask, server_subnet_mask=server_subnet_mask, buffer_size= buffer_size, number=3, name="Bottleneck")
    # set_logger(logger, level="INFO", message="Test 4 completed.\n  -----------------------------------------------------------------\n")
    set_logger(logger, level="INFO", message="\nConsolidated Result\n****************\nPath MTU of Client: " + str(cmtu) + "\nPath Mtu of server: " + str(smtu) + "\n RTT : " + str(rtt) + " us" +"\nTheoretical  Bottleneck Bandwidth: " + str(BB))

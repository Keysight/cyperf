...
import time
import datetime
#libpath = os.path.abspath(__file__+"/../../testsuite/rest_api_wrapper")
...
def create_new_config(server, username, password, client_id, config_path=None, config_name=None):
    """
    ...
    global rest
    rest = RESTasV3(ipAddress=server, username=username, password = password, client_id= client_id)
    config = config_path if config_path else None
    rest.setup(config, config_name)
    return rest

...
locals {
  ...
  startup_script = <<SCRIPT
                            /bin/bash /usr/bin/image_init_gcp.sh ${google_compute_instance.gcp_nats_instance.network_interface.0.network_ip} --username "${var.broker_username}" --password "${var.broker_password}" --fingerprint "">> image_init_behind_alb_log

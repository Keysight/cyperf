...
resource "aws_network_interface" "aws_broker_interface" {
    tags = {
...
data "aws_ami" "agent_ami" {
    ...
    filter {
      name   = "tag:Name"
      values = ["cyperf-agent-${var.agent_version}"]
    }
}
...
resource "aws_instance" "aws_broker" {
    tags = {
    ...
    network_interface {
        network_interface_id = aws_network_interface.aws_broker_interface.id
        device_index         = 0
    }

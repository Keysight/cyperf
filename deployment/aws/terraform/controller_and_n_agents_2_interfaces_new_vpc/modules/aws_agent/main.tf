locals{
    cli_agent_name = "client_${var.aws_stack_name}-${var.agent_role}-${var.agent_version}"
    srv_agent_name = "server_${var.aws_stack_name}-${var.agent_role}-${var.agent_version}"
}
...
resource "aws_instance" "aws_srv_agent" {
    tags = {
        Owner = var.tags.aws_owner
        Name = local.srv_agent_name
        Project = var.tags.project_tag
        Options = var.tags.options_tag
    }
    ami           = data.aws_ami.agent_ami.image_id 
    ...
    network_interface {
        ...
        network_interface {
        network_interface_id = aws_network_interface.aws_srv_test_interface.id
        device_index         = 1
    }
